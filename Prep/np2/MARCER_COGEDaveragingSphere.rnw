...
for (sid in 1:length(sub.ids)) { # sid<-1
  ...
  #     fname <- paste0(pp.path, sub.id, "/derivatives/fmriprep/sub-", sub.id, "/ses-1/func/sub-", sub.id, 
    ...
    for (rid in 1:length(run.ids)) { #rid<-2
      ...
      for (pid in 1:400) { #pid<-1
        for (i in 1:length(evt.vec)) { # i<-2
          ...
          TRs.to.avg[which(TRs.to.avg %in% censor.TRs)] <- NA;  # censor.TRs made from which desc-confounds_timeseries.tsv framewise_displacement rows > 0.9
          if (length(which(is.na(TRs.to.avg))) <= 5) {   # may not need this if: with na.rm=TRUE I think the mean

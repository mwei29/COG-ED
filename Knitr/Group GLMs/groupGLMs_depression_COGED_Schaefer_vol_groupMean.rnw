...
@
\noindent \texttt{groupGLMs\textunderscore depression\textunderscore COGED\textunderscore Schaefer\textunderscore vol\textunderscore groupMean.rnw} \par
...
\noindent For ANOVA test, Only significant results are shown in graph. \par
\vspace{0.2 cm} 
...
draw.plots <- function(which.set) {   # which.set <- "sch";
  ...
  f.eval <- readRDS(paste0(t.path,"anova_eval_", txt.lbl, "_vol.rds"))
  f.dec <- readRDS(paste0(t.path,"anova_dec_", txt.lbl, "_vol.rds"))
  ...
  for (pid in do.parcels) {   #  pid <- 1
    ...
    box();
    legend('bottomright', legend=c("MDD","HC"), col=c("dodgerblue","green"), pch=15, ncol=2, cex=0.7,  bty='n'); 
    ...
    legend(x=x.lim[1]-0.05, y=y.lim[2]-0.21, legend="MDD", col='black', lwd=2, ncol=1, cex=0.7,  bty='n');
    legend(x=x.lim[1]-0.05, y=y.lim[2]-0.28, legend="HC", col='black', lty='31', lwd=1.5, ncol=1, cex=0.7,  bty='n')

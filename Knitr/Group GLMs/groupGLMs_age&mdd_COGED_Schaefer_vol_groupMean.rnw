...
# MARCER COGED fMRI tasks, groupGLM results, showing both age & depression
# started 28 Dec 2023 by Emily Wei, adapting from Emily's template groupGLMs_depression_COGED_Schaefer_vol_groupMean.rmw
...
draw.plots <- function(which.set) {   # which.set <- "subcort";
  ...
  for (pid in do.parcels) {   #  pid <- 1
    ...
    for (i in 1:(4*length(valSV.lbls))) {      # i <- 3;
      vals <- valSV.lst[[i]][,paste0("Mean_",do.columns[pid])];
      if (!is.null(vals)) {
        if (length(vals) > x.lim[2]) { stop("length(vals) > x.lim[2]"); }
        }
        if (i <= 2){lines(x=0:(length(vals+1)), y=c(0,vals), col=clrs.valSV[i], lty='31', lwd=2);}
        if (2 < i & i <= 4) {lines(x=0:(length(vals+1)), y=c(0,vals), col=clrs.valSV[(i-2)], lwd=2);}
        if (4 < i & i <= 6) {lines(x=0:(length(vals+1)), y=c(0,vals), col=clrs.valSV[(i-4)], lty='31', lwd=1);}
        if (6 < i & i <= 8) {lines(x=0:(length(vals+1)), y=c(0,vals), col=clrs.valSV[(i-6)], lwd=1);}
    }

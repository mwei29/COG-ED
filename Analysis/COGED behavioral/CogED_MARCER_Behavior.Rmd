...
```{r Demo134, echo=F, warning=FALSE, message=FALSE,cache=F}
#Age
demo %>%
  group_by(Age.Group) %>%
  summarise(
...
### COGED: SV plot
``` {r CogED_SV_Plot, eval=T, echo=F, warning = F, message = F, cache=F}
...
```{r coged_fmri, eval = F, echo=F, warning=FALSE, message=FALSE, cache=F}
...
```{r H1, warning=F, message=F, cache=F}
#H1a
...
# Follow up analysis: only look at MDD participants, what's the age x load interaction?
...
# Follow up analysis: accuracy three way interaction predicting SV, but I found this problematic since accuracy is related to task load and task load is related to SV
...
```{r H2, warning=F, message=F, cache=F}
...
# Follow up analysis: age interaction in only MDD group
prox.choice.MDDonly <- coged.fmri.prox %>% 
  filter(MDD_Status == "MDD")
prox.choice.age.MDDonly <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*Age_centered + rewardCode + loadCode+ (1+proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
...
- Interaction between proximity² and MDD status (H2c). <br>
```{r H3, warning=F, message=F, cache=F}
...
# 
# # Plot the three-way interaction
# ggplot(efdata, aes(x = I(proximityValue^2), y = fit, color = factor(groupCode, levels = c(0, 1), labels = c("HC", "MDD")), group = groupCode)) + 
#     geom_line(size = 1.2) +
#     geom_ribbon(aes(ymin = fit - se, ymax = fit + se, fill = factor(groupCode, levels = c(0, 1), labels = c("HC", "MDD"))), alpha = 0.3) + 
#     labs(x = "Decision difficulty (higher value = easier decision)", y = "zRT", 
#          color = "MDD Status", fill = "MDD Status") +
#     theme_classic() +
#     facet_wrap(~ Age_centered)
# 
# ggplot(efdata, aes(x = I(proximityValue^2), y = fit, 
#                    color = Age_centered, 
#                    group = Age_centered)) + 
#   geom_line(size = 1.2) +
#   geom_ribbon(aes(ymin = fit - se, ymax = fit + se, 
#                   fill = Age_centered), 
#               alpha = 0.3) + 
#   labs(x = "Decision difficulty (higher value = easier decision)", 
#        y = "zRT", 
#        color = "Age", 
#        fill = "Age") +
#   theme_classic() +

...
source("C:/Users/Emotion/Documents/GIT-MW/COG-ED/Tools/summarySEwithin2.R")
# library(boxr); box_auth(); 

in.path <- "C:/Users/Emotion/Documents/GIT-MW/COG-ED/Data/" 
out.path <-"C:/Users/Emotion/Documents/GIT-MW/COG-ED/Analysis/COGED behavioral/" 
dir.desc <- "20250114";  
...
highSV <- coged.SV[coged.SV$meanSV > 0.8 & coged.SV$task == "Purple", ,drop = FALSE]
highSV.sid <- unique(highSV$subID)
highSV.demo <- demo[demo$sub.id %in% highSV.sid,]
highSV.demo %>%
  count(Race,Age.Group) %>%
  mutate(Prop = n / sum(n))
...
SV.group <- lmer(data = coged.lab, SV ~ taskCode + Age_centered + groupCode + (1 | subID))
summary(SV.group)
```
...
nback.fmri <- nback.fmri %>% filter(resp != 9) %>%select(subID, level, kind, acc, RT) %>%
  mutate(level = factor(level, levels = c(1,2,3,4), labels = c("Black","Red","Blue","Purple")),
         ...
         FA = if_else((kind == 0 & acc == 0), 1, 0)) %>%
  filter(type != "lure") %>%
  left_join(demo.scan %>% select(sub.id, MDD_Status, Age.Group, Age_centered, groupCode), 
            by = c("subID" = "sub.id")) %>%
  group_by(subID) %>%
  mutate(RT_pm = mean(RT,na.rm=TRUE),
         RT_sd = sd(RT,na.rm=TRUE)) %>%
...
coged.fmri <- coged.fmri %>% filter(trial > 45) %>% filter(choice != 9) %>%
  ...
  mutate(trialCode = factor (trialType, levels = c("low-catch","low-biased","high-biased","high-catch"),labels = c(0,1,2,3))) %>%
    group_by(subID) %>%
  mutate(RT_pm = mean(choiceRT,na.rm=TRUE),
         RT_sd = sd(choiceRT,na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(zRT = (choiceRT-RT_pm)/RT_sd)
...
coged.fmri.prox <- coged.fmri %>%
  ...
  select(subID, Trial, Run, choice, proximityValue, choseHard,loadCode, rewardCode, Age_centered, groupCode, zRT, choiceRT,MDD_Status) %>% as_tibble()

...
#####4/16/25 this is as much as I have updated the code
...
ef <- effects::effect(term="taskCode*Age_centered", xlevels= list(Age_centered=c(42.29, 63.85)), mod=SV.age)
...
SV.rew.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode*groupCode + Age + acc + (1 | subID))
summary(SV.rew.MDD)

SV.acc.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode + Age + acc*MDD_Status + (1 | subID))
summary(SV.acc.MDD)

SV.acc.age <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode + MDD_Status + acc*Age + (1 | subID))
summary(SV.acc.age)
...
ef <- effects::effect(term="acc*Age", xlevels= list(Age=c(42.29, 63.85)), mod=SV.acc.age)
efdata<-as.data.frame(ef) #convert the effects list to a data frame
efdata$Age<-as.factor(efdata$Age)
ggplot(efdata, aes(x=acc, y=fit, color=Age,group=Age)) + 
    geom_point() + 
    geom_line(size=1.2) +
    geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=Age),alpha=0.3) + 
...
prox.choice <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue + Age + groupCode + (proximityValue|subID), family = "binomial")
summary(prox.choice)
...
prox.choice.age <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*Age + groupCode + (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
summary(prox.choice.age)

#H2c: MDD interaction N.S.
prox.choice.MDD <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*groupCode + Age + (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
...
prox.RT <- lmer(data = coged.fmri.prox, choiceRT ~ I(proximityValue^2) + Age + groupCode + (I(proximityValue^2)|subID))
summary(prox.RT)

#H2b: In older adults, the negative effect of proximity² on RT is weaker.
prox.RT.age <- lmer(data = coged.fmri.prox, choiceRT ~ I(proximityValue^2)*Age + groupCode + (I(proximityValue^2)|subID))
summary(prox.RT.age)

#H2c: MDD interaction N.S.
prox.RT.MDD <- lmer(data = coged.fmri.prox, choiceRT ~ I(proximityValue^2)*groupCode + Age + (I(proximityValue^2)|subID))
summary(prox.RT.MDD)

#Exploratory: age & MDD interaction. N.S.
prox.RT.age.MDD <- lmer(data = coged.fmri.prox, choseHard ~ I(proximityValue^2)*Age*groupCode + (I(proximityValue^2)|subID))

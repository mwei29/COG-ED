...
### Demoographic summary with 134 participants that have done some scans
...
nback.lab  <- nback.lab  %>% filter(subID %in% sub.ids)
length(unique(nback.lab$subID))
nback.fmri <- nback.fmri %>% filter(subID %in% sub.ids)
...
coged.lab <- coged.lab  %>% filter(subID %in% sub.ids)
...
#H1a
SV <- lmer(data = coged.lab.SV, SV ~ taskCode + Age_centered + groupCode + acc + (1 | subID))
summary(SV)

#H1b
SV.age <- lmer(data = coged.lab.SV, SV ~ taskCode*Age_centered + groupCode + acc + (1 | subID))
summary(SV.age)
...
ggplot(efdata, aes(x=taskCode, y=fit, color=Age,group=Age)) + 
    ...
    scale_x_continuous(breaks = seq(-1, 1, by=0.5), limits=c(-1, 1))
#H1c
SV.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode*groupCode + Age_centered + acc + (1 | subID))
summary(SV.MDD)
...
SV.age.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode*Age_centered*groupCode + acc + (1 | subID))
summary(SV.age.MDD)
...
SV.rew <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode + Age_centered + groupCode + acc + (1 | subID))
summary(SV.rew)

#looking at interaction between MDD & Reward
SV.rew.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode*groupCode + Age_centered + acc + (1 | subID))
summary(SV.rew.MDD)

SV.acc.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode + Age_centered + acc*MDD_Status + (1 | subID))
summary(SV.acc.MDD)

SV.acc.age <- lmer(data = coged.lab.SV, SV ~ taskCode + rewardCode + MDD_Status + acc*Age_centered + (1 | subID))
summary(SV.acc.age)

ef <- effects::effect(term="acc*MDD_Status", xlevels= list(MDD_Status = c(0,1)), mod=SV.acc.MDD)
efdata<-as.data.frame(ef) #convert the effects list to a data frame
efdata$MDD_Status<-as.factor(efdata$MDD_Status)
ggplot(efdata, aes(x=acc, y=fit, color=MDD_Status,group=MDD_Status)) + 
    geom_point() + 
    geom_line(size=1.2) +
    geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=MDD_Status),alpha=0.3) + 
    labs(x= "N-back Accuracy", y="Subjective Value", color="MDD_Status", fill="MDD_Status") +
    theme_classic() + theme(text=element_text(size=12)) + 
...
prox.choice <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue + Age_centered + groupCode + (proximityValue|subID), family = "binomial")
summary(prox.choice)
prox.choice1 <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue+ rewardCode + loadCode + Age_centered + groupCode + (proximityValue|subID), family = "binomial")
...
preds.group <- preds %>%
  left_join(demo %>% select(sub.id, MDD_Status, Age.Group), by = c("subID" = "sub.id"))
...
prox.choice.age <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*Age_centered + groupCode + (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
summary(prox.choice.age)
prox.choice.age1 <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*Age_centered + groupCode + rewardCode + loadCode+ (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
summary(prox.choice.age1)

#H2c: MDD interaction N.S.
prox.choice.MDD <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*groupCode + Age_centered + (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
summary(prox.choice.MDD)
prox.choice.MDD1 <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*groupCode + Age_centered + rewardCode + loadCode+ (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
...
prox.choice.age.MDD <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*Age_centered*groupCode + (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")
summary(prox.choice.age.MDD)
prox.choice.age.MDD1 <- glmer(data = coged.fmri.prox, choseHard ~ proximityValue*Age_centered*groupCode + rewardCode + loadCode + (proximityValue|subID), control = glmerControl(optimizer = "bobyqa"), family = "binomial")

...
coged.lab$Age <- coged.lab$Age <- as.numeric(as.character(coged.lab$Age))
...
coged.fmri.clean.prox$loadCode <- as.numeric(as.character(coged.fmri.clean.prox$loadCode))
coged.fmri.clean.prox$rewardCode <- as.numeric(as.character(coged.fmri.clean.prox$rewardCode))
...
#plot
fig2 <- ggplot(fixed_pred, aes(x=proximityValue, y=Predicted)) + 
  geom_ribbon(data = fixed_pred, aes(x = proximityValue, ymin = CI_low, ymax = CI_high), alpha = 0.5) +
  geom_line(data = fixed_pred, aes(x = proximityValue, y = Predicted), linewidth = 2) +
  geom_line(data = preds.group, aes(x=proximityValue, y=Predicted, group=interaction(subID), colour = group), alpha=0.5) + theme_classic() + labs(x= "Relative Subjective Value of High-Effort Option", y="Proabability of High-Effort Option") +
  scale_color_manual(values = c("OA_MDD" = "#E63946",  
                                "MA_MDD" = "#F4A261",  
                                "OA_HC"  = "#457B9D", 
                                "MA_HC"  = "#2A9D8F")) 
fig.group <- fig2 + 
  guides(colour = guide_legend(title = "MDD and Age Status", override.aes = list(linewidth = 2))) +
...
```{r H1, warning=F, message=F, cache=TRUE}
...
#H1a: as hypothesized: As task difficulty increases, subjective value (SV) decreases.
SV <- lmer(data = coged.lab.SV, SV ~ taskCode + Age + MDD_Status + acc + (1 | subID))
summary(SV)
#H1b: Unexpectedly, age interaction (t=2.031) shows that as age increases, the negative effect of N-back level on SV diminishes.
SV.age <- lmer(data = coged.lab.SV, SV ~ taskCode*Age + MDD_Status + acc + (1 | subID))
summary(SV.age)
...
#H1c: Unexpectedly, MDD interaction (t=2.105) shows that for MDD group, the negative effect of N-back level on SV diminishes.
SV.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode*MDD_Status + Age + acc + (1 | subID))
summary(SV.MDD)
ef <- effects::effect(term="taskCode*MDD_Status", xlevels= list(MDD_Status = levels(coged.lab$MDD_Status)), mod=SV.MDD)
efdata<-as.data.frame(ef) #convert the effects list to a data frame
efdata$MDD_Status<-as.factor(efdata$MDD_Status)
ggplot(efdata, aes(x=taskCode, y=fit, color=MDD_Status,group=MDD_Status)) + 
    geom_point() + 
    geom_line(size=1.2) +
    geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=MDD_Status),alpha=0.3) + 
    labs(x= "N-back Difficulty", y="Subjective Value", color="MDD_Status", fill="MDD_Status") +
    theme_classic() + theme(text=element_text(size=12)) + 
    scale_y_continuous(breaks = seq(0, 1, by=0.1), limits=c(0.3,0.8)) +
    scale_x_continuous(breaks = seq(-1, 1, by=0.5), limits=c(-1, 1))

#Exploratory: age & MDD interaction. 
SV.age.MDD <- lmer(data = coged.lab.SV, SV ~ taskCode*Age*MDD_Status + acc + (1 | subID))
summary(SV.age.MDD)
...
ef <- effects::effect("taskCode*Age*MDD_Status", 
             mod = SV.age.MDD, 
             xlevels = list(Age = c(age_mean - age_sd, age_mean, age_mean + age_sd)))
...
A linear mixed-effects model examined the effects of **N-back level**, **age (cont.)**, and **MDD status (MDD vs. HC)** on **SV**, with **N-back Accuracy** being covariate.
...
#The larger the proximity², the easier the choice is
coged.fmri.clean.prox <- coged.fmri.clean.prox %>%
  mutate(proximity_sq = proximityValue^2)
#H3a: proximity² did not significantly predict RT
prox.RT <- lmer(data = coged.fmri.clean.prox, choiceRT ~ proximity_sq + Age + groupCode + (proximity_sq|subID))
summary(prox.RT)

#H2b: In older adults, the negative effect of proximity² on RT is weaker.
prox.RT.age <- lmer(data = coged.fmri.clean.prox, choiceRT ~ proximity_sq*Age + groupCode + (proximity_sq|subID))
summary(prox.RT.age)

#H2c: MDD interaction N.S.
prox.RT.MDD <- lmer(data = coged.fmri.clean.prox, choiceRT ~ proximity_sq*groupCode + Age + (proximity_sq|subID))
summary(prox.RT.MDD)

#Exploratory: age & MDD interaction. N.S.
prox.RT.age.MDD <- lmer(data = coged.fmri.clean.prox, choseHard ~ proximity_sq*Age*groupCode + (proximity_sq|subID))

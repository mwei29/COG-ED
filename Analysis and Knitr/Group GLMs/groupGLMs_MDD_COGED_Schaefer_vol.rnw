...
glm.path <- "/scratch2/weie/COGED_CODE/MDD_parcelStats/COGED_parcelStats/";  
...
@
\noindent \texttt{groupGLMs\textunderscore MDD\textunderscore COGED\textunderscore Schaefer\textunderscore vol.rnw} \par
\noindent compiled \today\  \par
\noindent \textbf{group} MARCER MDD COGED group GLM results, volumes. \par
\vspace{0.2 cm} 
...
draw.plots <- function(which.set) {   # which.set <- "sch";
  ...
  for (i in 1:length(cnc.lbls)) {      # rid <- 1;
    fname <- paste0(glm.path, "COGED_CatchNonCatch_", cnc.lbls[i], "_", txt.lbl, "_vol_groupT.txt");

...
# MARCER COGED fMRI tasks, groupGLM results,OA and MA difference
# started 20 Dec 2023 by Emily Wei, adapting from Emily's template groupGLMs_depression_COGED_Schaefer_vol_groupMean.rmw
...
group.lbls <- c("OA","MA") #group.lbls <- "OA"
...
@
\noindent \texttt{groupGLMs\textunderscore age\textunderscore COGED\textunderscore Schaefer\textunderscore vol\textunderscore groupMean.rnw} \par
\noindent compiled \today\  \par
\noindent \textbf{group} MARCER OA and MA COGED group GLM results, volumes. \par
\noindent Y axis is the group mean BOLD response \par
\noindent There're 39 OA and 40 MA included in this analysis. \par
\noindent Robust unpaired t test was ran to test the difference between OA's and MA's meanResp in evaluation phase and decision making phase. \par
\noindent Evaluation phase is set between knots 3-5; decision making phase is set between knots 10-13. \par
\noindent 2 by 3 ANOVA test was ran to test the effect of group and reward to BOLD in evaluation phase and decision making phase. \par
\noindent For both robust t test and ANOVA test, Only significant results are shown in graph. \par
\vspace{0.2 cm} 
...
draw.plots <- function(which.set) {   # which.set <- "subcort";
  ...
  f.eval <- readRDS(paste0(t.path,"anova_eval_age_", txt.lbl, "_vol.rds"))
  f.dec <- readRDS(paste0(t.path,"anova_dec_age_", txt.lbl, "_vol.rds"))
  ...
  t <- read.delim(paste0(t.path, txt.lbl, "_vol_robustttest_age.txt"))
  ...
  for (pid in do.parcels) {   #  pid <- 1
    ...
    box();
    legend('bottomright', legend=c("OA","MA"), col=c("dodgerblue","green"), pch=15, ncol=2, cex=0.7,  bty='n'); 
    ...
    legend(x=x.lim[1]-0.05, y=y.lim[2]-0.21, legend="OA", col='black', lwd=2, ncol=1, cex=0.7,  bty='n');
    legend(x=x.lim[1]-0.05, y=y.lim[2]-0.28, legend="MA", col='black', lty='31', lwd=1.5, ncol=1, cex=0.7,  bty='n')
    ...
    for (vid in 2:4) {   # vid <- 2; pid <- 1
      ...
      # vals_tbl <- rbind(rewload.lst[[inds[1]]][,paste0("Mean_",do.columns[pid])],
      #                   rewload.lst[[inds[2]]][,paste0("Mean_",do.columns[pid])],
      #                   rewload.lst[[inds[3]]][,paste0("Mean_",do.columns[pid])],
      #                   rewload.lst[[inds[4]]][,paste0("Mean_",do.columns[pid])],
      #                   rewload.lst[[inds[5]]][,paste0("Mean_",do.columns[pid])],
      #                   rewload.lst[[inds[6]]][,paste0("Mean_",do.columns[pid])]);
      vals_OA <- apply(vals_tbl[,1:3], 1, mean);  # average across the three instances
      vals_MA <- apply(vals_tbl[,4:6], 1, mean);
      lines(x=0:(length(vals_MA+1)), y=c(0,vals_MA), col=clrs.rew[inds[1]], lty='31', lwd=1.5);
      lines(x=0:(length(vals_OA+1)), y=c(0,vals_OA), col=clrs.rew[inds[1]], lwd=2);
    }
    legend('topleft', legend=paste0("Rew", 2:4), col=clrs.rew[1:3], lwd=2, ncol=3, cex=0.7,  bty='n');
    #group effect
    if (f.eval[1,5,pid] < 0.05) {
    text(x=x.lim[1]-0.05, y=y.lim[2]-0.7, labels=paste0("group:F=", round(f.eval[1,4,pid], 3)), pos=4, cex=0.65);
    text(x=x.lim[1]-0.05, y=y.lim[2]-0.75, labels=paste0("p=", round(f.eval[1,5, pid], 3)), pos=4, cex=0.65); }
    if (f.dec[1,5,pid] < 0.05) {
    text(x=x.lim[2]-15, y=y.lim[2]-0.7, labels=paste0("group:F=", round(f.dec[1,4,pid], 3)), pos=4, cex=0.65);
    ...
    #interaction
    if (f.eval[4,5,pid] < 0.05) {
    text(x=x.lim[1]-0.05, y=y.lim[2]-0.6, labels=paste0("group:reward:F=", round(f.eval[4,4,pid], 3)), pos=4, cex=0.65);
    text(x=x.lim[1]-0.05, y=y.lim[2]-0.65, labels=paste0("p=", round(f.eval[4,5, pid], 3)), pos=4, cex=0.65); }
    if (f.dec[4,5,pid] < 0.05) {
    text(x=x.lim[2]-15, y=y.lim[2]-0.1, labels=paste0("group:reward:F=", round(f.dec[4,4,pid], 3)), pos=4, cex=0.65);
    text(x=x.lim[2]-15, y=y.lim[2]-0.15, labels=paste0("p=", round(f.dec[4,5, pid], 3)), pos=4, cex=0.65);}
    ...
    for (vid in c("EV_Low_", "EV_Medium_", "EV_High_")) {   # vid <- "EV_Low_";
      ...
      lines(x=0:(length(vals_MA+1)), y=c(0,vals_MA), col=clrs.load[inds[1]], lty='31', lwd=1.5);
      lines(x=0:(length(vals_OA+1)), y=c(0,vals_OA), col=clrs.load[inds[1]], lwd=2);
    }
    ...
    if (t[2,pid] < 0.05) {
    text(x=x.lim[1]-0.05, y=y.lim[2]-0.6, labels=paste0("t=", round(t[1,pid], 3)), pos=4, cex=0.7);
    text(x=x.lim[1]-0.05, y=y.lim[2]-0.7, labels=paste0("p=", round(t[2,pid], 3)), pos=4, cex=0.7)};
    if (t[4,pid] < 0.05) {
    text(x=x.lim[2]-15, y=y.lim[2]-0.6, labels=paste0("t=", round(t[3,pid], 3)), pos=4, cex=0.7);
    text(x=x.lim[2]-15, y=y.lim[2]-0.7, labels=paste0("p=", round(t[4,pid], 3)), pos=4, cex=0.7)};
